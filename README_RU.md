...
Страница проекта: [rsa-labs.ru/computer-vision/video-ai-editor](https://rsa-labs.ru/computer-vision/video-ai-editor/)

Десктопное приложение для чернового видеомонтажа: локальный семантический анализ
медиатеки (Qwen3.5 через Ollama), таймлайн-редактор, предпросмотр и экспорт через
ffmpeg, плюс LLM-агент, который собирает монтаж по текстовому запросу («сделай
динамичный ролик на минуту из кадров с морем под этот трек»).
...
- **Анализ медиа локальной VL-моделью** (Qwen3.5 через Ollama) — описание, объекты, стиль,
  настроение; для видео — сцены с таймкодами, по которым можно выбрать нужный фрагмент
  внутри длинного файла. Каждый кадр и файл получает оценку технического качества
  (высокое / среднее / низкое — тёмный, размытый, смазанный движением или зашумлённый
  кадр), и агент отсеивает плохой материал по данным, а не по просьбе. Очередь с
  прогрессом, отменой и быстрым режимом на модели поменьше.
...
- **Экспорт** — MP4/H.264 (CRF 18/23/28 или битрейт), MOV/ProRes, WebM/VP9; прогресс с
  оценкой времени, отмена, проверка свободного места. Итоговый микс приводится к
  −14 LUFS (уровень стриминговых платформ) и в превью, и в экспорте — проекты на
  треках разного мастеринга не скачут по громкости между собой.
- **Агент-монтажёр** — собирает черновой монтаж по текстовому запросу через внешнюю
  ...
  (эмбеддинги описаний), лог действий в интерфейсе. Склейки ложатся на тактовую
  сетку, переходы — только на границы фраз, которые в музыке действительно есть,
  материал отсеивается по оценке качества — всё по посчитанным данным, а не по
  пожеланиям в промпте.
- **MCP-сервер** — те же инструменты монтажа для внешнего агента (Claude Code,
  Claude Desktop и др.).
- **Панель настроек, журнал, уведомления** — все параметры `.env` правятся из
  интерфейса (backend перечитывает их на лету), проверка окружения говорит, чего не
  хватает и что делать, журнал backend открывается одной кнопкой, а завершение
  фоновых задач (скан, анализ, экспорт, агент) показывается тостом — и системным
  уведомлением, если окно не на переднем плане. Описания и теги в `meta.json` можно
  поправить руками.
- **Десктопная оболочка (Tauri)** — один запуск вместо двух терминалов: окно само
  поднимает backend из `.venv`, несёт свои статические ffmpeg/ffprobe/ffplay, даёт
  нативный выбор папок и останавливает backend при закрытии. Режим браузера остаётся.
...
# Apple Silicon (сборка MLX); на Linux/Windows — без суффикса -mlx: qwen3.5:4b, qwen3.5:0.8b
...
| `VL_MODEL`, `VL_MODEL_FAST` | модели анализа в Ollama (обычная и быстрая); `qwen2.5vl:7b` тоже работает, но втрое медленнее |
...
Помимо требований выше нужен Rust (`rustup`). Один раз:

```bash
./scripts/fetch-ffmpeg.sh          # статические ffmpeg/ffprobe/ffplay в frontend/src-tauri/binaries
```
...
Окно запускает `.venv/bin/python -m uvicorn` из корня репозитория, ждёт его и
останавливает при выходе; уже поднятый на этом порту backend переиспользуется. Вшитые
бинарники передаются backend как `FFMPEG_PATH` и т. п. и имеют приоритет над `.env` —
панель настроек такие поля помечает. `npm run tauri build --prefix frontend` собирает
`.app`/`.dmg` в `frontend/src-tauri/target/release/bundle/`; собранное приложение
по-прежнему поднимает backend из `.venv` этого репозитория (путь вшивается при сборке,
переопределяется `IVE_PROJECT_ROOT`) — полностью автономная сборка со встроенным Python
...
1. **Добавить папки** медиатеки (⚙ → Хранилище, или прописать `MEDIA_ROOTS`) и дождаться
...
2. **Проанализировать файлы** — кнопка анализа разбирает отмеченные файлы через
   VL-модель и складывает результат в `meta.json`. Повторный анализ с перезаписью —
   отдельной кнопкой; он нужен, если разметка устарела (например, после обновления
   у аудио появился темп, а у кадров видео — оценка качества).
...
такие тесты пропускаются. `test_e2e.py` проходит весь конвейер разом — скан →
анализ (VL подменена, трекинг долей настоящий) → инструменты агента → сохранение →
экспорт — через те же функции, что зовёт приложение.
...
frontend/src-tauri/  оболочка Tauri: запуск backend, вшитый ffmpeg
scripts/          fetch-ffmpeg.sh — статические бинарники для оболочки
...
экспорт, агент-монтажёр с MCP-сервером, панели настроек и журнала, оболочка Tauri)
реализована и покрыта тестами. Оболочка запускает backend из `.venv` репозитория, а
не встраивает Python; автономный инсталлятор не планируется, пока у приложения один
пользователь.
